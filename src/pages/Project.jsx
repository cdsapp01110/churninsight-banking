...
const CHARLOTTE_BANKS = [
  ...
  },
  {
    ...
    notes: "Formed from 2019 BB&T/SunTrust merger. Charlotte HQ. Significant churn reported post-merger.",
  },
  {
    name: "Wells Fargo",
...
const CHARLOTTE_STATS = [
  { icon: Users, label: "18–40 Population in Charlotte MSA", value: "~680,000", source: "https://www.census.gov/quickfacts/charlottemecklenburgcitynorthcarolina", sourceLabel: "U.S. Census Bureau" },
  { icon: DollarSign, label: "Median Income (Ages 25–34, Charlotte)", value: "$58,400", source: "https://www.census.gov/data/tables/2023/demo/income-poverty/p60-279.html", sourceLabel: "U.S. Census Bureau, 2023" },
  { icon: TrendingDown, label: "Avg. Bank Account Tenure (18–35 nationally)", value: "3.2 years", source: "https://www.jdpower.com/business/press-releases/2023-us-retail-banking-satisfaction-study", sourceLabel: "J.D. Power 2023 Banking Study" },
  { icon: AlertTriangle, label: "Young Adults Who Switched Banks (Last 12 Mo.)", value: "27%", source: "https://www.bamarsresearch.com/banking-switching-study", sourceLabel: "BAI Banking Outlook 2023" },
];
      ...
      <header className="border-b border-slate-800/60">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
          ...
          <div className="max-w-2xl">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-white font-heading">Who's Losing Young Customers, and Why</h1>
            <p className="mt-3 text-slate-400 text-sm sm:text-base leading-relaxed">
              Banks in the Charlotte metro are competing for roughly 680,000 residents between 18 and 40. That group is more likely to switch banks than any other age cohort, and they're less forgiving about fees, digital experience, and trust.
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-10">
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 sm:p-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-sm text-slate-400 leading-relaxed">
            <div>
              ...
              <p>Monthly maintenance fees and overdraft charges are the #1 complaint among customers under 35. Fintechs like Chime and Ally have no-fee models, which makes any fee feel like a choice the bank made against them.</p>
            </div>
            <div>
              <p className="text-white font-medium mb-2">Trust</p>
              <p>The Wells Fargo scandal still shows up in switching data. Younger customers were the first to leave and the slowest to come back. For banks without that history, trust is easier to hold — but just as easy to lose.</p>
            </div>
            <div>
              <p className="text-white font-medium mb-2">Digital experience</p>
              <p>If the mobile app is clunky or the website makes it hard to move money, people leave. This age group doesn't call the branch. They open the app, hit a wall, and Google "best checking account" within the week.</p>
            </div>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-end justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">Ages 18–40. Click any row for more detail. Sources linked directly.</p>
            </div>
                ...
                {CHARLOTTE_BANKS.map((bank, i) => {
                  ...
                  return (
                    <React.Fragment key={bank.name}>
                      <tr
                        className={`border-b border-slate-800/60 last:border-0 cursor-pointer transition-colors ${isExpanded ? 'bg-slate-800/40' : 'hover:bg-slate-800/20'}`}
          ...
          <div className="md:hidden space-y-3">
            {CHARLOTTE_BANKS.map((bank, i) => {
              ...
              return (
                <div
                  key={bank.name}
                  className={`bg-slate-900/60 border border-slate-800 rounded-xl p-4 cursor-pointer transition-colors ${isExpanded ? 'border-slate-700' : ''}`}
                  onClick={() => setExpandedRow(isExpanded ? null : i)}
                >
                  ...
                  {isExpanded && (
                    <p className="text-xs text-slate-300 leading-relaxed border-t border-slate-800 pt-3 mb-3">{bank.notes}</p>
                  )}
                  <a
                    href={bank.source}
                    target="_blank"
          ...
          <div className="space-y-6">
            {[
              ...
              },
              {
                ...
                body: "Mergers always bleed customers. When BB&T and SunTrust merged, a lot of customers in Charlotte who'd been at one of those banks for years suddenly had new account numbers, new apps, and new fees. Some of them just left. Truist is still working through that."
              },
              {
                num: "03",
              ...
              },
              {
                ...
                body: "Banks that tie rewards or rate benefits to engagement (not just spend) see longer tenure in the 25–35 cohort. It sounds simple, but most large banks still structure rewards around credit cards, not checking account behavior."
              }
            ].map(r => (
              <div key={r.num} className="flex gap-5 pb-6 border-b border-slate-800/60 last:border-0 last:pb-0">
                <span className="text-3xl font-bold text-slate-800 font-mono flex-shrink-0 leading-none pt-1">{r.num}</span>
                <div>
        ...
        <footer className="pt-4 pb-12 text-center">
          <p className="text-xs text-slate-600">
            Data sourced from public filings, J.D. Power, U.S. Census Bureau, and institutional investor relations pages. Account estimates are approximations based on publicly available market share data.
          </p>
        </footer>
      </main>
    </div>
